...
export const GlareCard = ({
  ...
  const glareHostRef = useRef<HTMLDivElement>(null);
  const state = useRef({
    glare: {
      x: 50,
      y: 50,
  ...
  return (
    <div
      ...
      onPointerEnter={() => {
        ...
        glareHostRef.current?.style.setProperty("--opacity", "0.6");
        if (refElement.current) {
          setTimeout(() => {
      ...
      onPointerLeave={() => {
        ...
        glareHostRef.current?.style.setProperty("--opacity", "0");
        if (refElement.current) {
    ...
    >
      <div
        ref={glareHostRef}
        className="h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] border border-slate-800 hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden"
      >
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))]">
          {imageUrl && (
            <img
              src={imageUrl}
              alt="Background"
              className="absolute inset-0 h-full w-full object-cover [clip-path:inset(0_0_0_0_round_var(--radius))]"
            />
          )}
          <div
            className={cn(
